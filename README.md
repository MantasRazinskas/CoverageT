...
5. Reports, at thresholds `0.70`, `0.75`, `0.80`, `0.85`:
   - **Recall (coverage)** = `TP_reference / total_reference`
   - **Precision** = `TP_generated / total_generated` (any-match policy)
...
The user stories evaluated in the paper were generated by giving the prompt in
`prompts/user_story_generation_prompt.md` to an LLM together with the project
artifacts. The expert (reference) stories were written independently. Running
this tool on those two sets reproduces the coverage and recall / precision / F1
